...
// Update a previous standard account, including optional password reset.
app.put("/api/auth/users/:id", requireAuth, requireSuperAdmin, async (req, res) => {
  try {
    if (!mongoose.Types.ObjectId.isValid(req.params.id)) return res.status(400).json({ message: "Invalid account ID" });
    const user = await User.findById(req.params.id);
    if (!user) return res.status(404).json({ message: "Account not found" });
    if (isSuperAdmin(user)) return res.status(400).json({ message: "Super Admin account cannot be changed here" });

    const fullName = String(req.body.fullName || "").trim().slice(0, 80);
    const username = normalizeUsername(req.body.username);
    const newPassword = String(req.body.password || "");
    if (!fullName) return res.status(400).json({ message: "Full name is required" });
    if (!/^[a-z0-9._-]{3,50}$/.test(username)) return res.status(400).json({ message: "Enter a valid username" });
    if (await User.exists({ username, _id: { $ne: user._id } })) return res.status(409).json({ message: "Username already exists" });
    ...
    await user.save();
    res.json({ message: "Previous user updated successfully", user: safeUser(user) });
  } catch (error) {
    console.error("Existing user update error:", error);
    if (error && error.code === 11000) return res.status(409).json({ message: "Username already exists" });
    res.status(500).json({ message: "Unable to update previous user" });
  }
});

// Permanently delete a standard account. Super Admin accounts are protected.
app.delete("/api/auth/users/:id", requireAuth, requireSuperAdmin, async (req, res) => {
  try {
    if (!mongoose.Types.ObjectId.isValid(req.params.id)) return res.status(400).json({ message: "Invalid account ID" });
    const user = await User.findById(req.params.id);
    if (!user) return res.status(404).json({ message: "Account not found" });
    if (isSuperAdmin(user)) return res.status(400).json({ message: "Super Admin account cannot be deleted" });
    if (String(user._id) === String(req.authUser._id)) return res.status(400).json({ message: "You cannot delete your currently signed-in account" });

    const deletedUsername = user.username;
    await User.deleteOne({ _id: user._id });
    res.json({ message: `Account '${deletedUsername}' deleted successfully` });
  } catch (error) {
